...
def get_messages(conversation_id):
    ...
    message_ids = [m["_id"] for m in messages if user_id_str not in m.get("delivered_to", [])]
    if message_ids:
        mongo.db.messages.update_many(
            {"_id": {"$in": message_ids}},
            {"$addToSet": {"delivered_to": user_id_str}},
        )
...
def send_message(conversation_id):
    ...
    if reply_to_message_id:
        msg_doc["reply_to_message_id"] = str(reply_to_message_id)
    ...
    serialized_message = serialize(msg_doc)
    socketio.emit(
        "message:new",
        {
            "conversation_id": conversation_id,
    ...
    socketio.emit(
        "conversation:unread",
        {"user_id": other_user_id, "count_delta": 1},
...
def mark_conversation_read(conversation_id):
    ...
    if message_ids:
        mongo.db.messages.update_many(
            {"_id": {"$in": message_ids}},
            {"$set": {"is_read": True}, "$addToSet": {"read_by": user_id_str}},
        )

    mongo.db.conversations.update_one(
        {"_id": ObjectId(conversation_id)},
        {"$set": {f"unread_counts.{user_id_str}": 0}},
    )
    ...
    return jsonify({'ok': True, 'read_count': len(message_ids)}), 200
